...
async def _seed_plan_with_data(session: AsyncSession, org_id: str, user_id: str) -> Plan:
    ...
    prod = Product(
        organization_id=org_id, name="P", sku=f"SKU-{suffix}", unit_price=50, unit_cost=25
    )
    ...
    period = TimePeriod(organization_id=org_id, year=2025, month=1, quarter=1, label="2025-Q1")
...
async def test_agent_workflow_end_to_end(client: AsyncClient, session: AsyncSession) -> None:
    ...
    resp = await client.post(
        "/api/v1/agents/plan",
        headers=headers,
        json={"plan_id": plan_id, "goal": "Increase demand by 30% and keep headcount unchanged."},
    )
    ...
    change_sets = (
        (await session.execute(select(ChangeSet).where(ChangeSet.organization_id == org.id)))
        .scalars()
        .all()
    )
    ...
    resp = await client.post(
        ...
        headers=headers,
        json={"reason": "recommended scenario meets the bar"},
    )
...
async def test_knowledge_is_tenant_scoped(client: AsyncClient, session: AsyncSession) -> None:
    ...
    resp_a = await client.get(
        "/api/v1/knowledge/search", headers=headers_a, params={"q": "pricing"}
    )
    assert resp_a.status_code == 200
    assert resp_a.json()["chunks"]
    resp_b = await client.get(
        "/api/v1/knowledge/search", headers=headers_b, params={"q": "pricing"}
    )
